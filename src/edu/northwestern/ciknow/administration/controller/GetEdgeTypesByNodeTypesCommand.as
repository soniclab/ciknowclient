package edu.northwestern.ciknow.administration.controller
{
	...
	public class GetEdgeTypesByNodeTypesCommand
	{
		private static var logger:ILogger = LogUtil.getLog(GetEdgeTypesByNodeTypesCommand);
		...
		public function GetEdgeTypesByNodeTypesCommand()
		{
		}
		
		public function execute(event:EdgeEvent):AsyncToken{
			logger.info("Get edge types by node types...");
			return service.getEdgeTypesByNodeTypes(event.data);
		}
		
		public function result(event:ResultEvent, trigger:EdgeEvent):void{		
			logger.info("Got edge types by node types.");
			var e:EdgeEvent = new EdgeEvent(EdgeEvent.GOT_EDGE_TYPES_BY_NODE_TYPES);
			e.data = new Object();
