package edu.northwestern.ciknow.administration.controller
{
	...
	public class DeriveEdgesByContactChooserCommand
	{
		private static var logger:ILogger = LogUtil.getLog(DeriveEdgesByContactChooserCommand);
		...
		public function DeriveEdgesByContactChooserCommand()
		{
		}
		
		public function execute(event:EdgeEvent):AsyncToken{
			logger.info("Deriving edges by contact chooser...");
			return service.deriveEdgesByCC(event.data);
		}
		
		public function result(event:ResultEvent, trigger:EdgeEvent):void{		
			logger.info("Derived edges by contact chooser.");
