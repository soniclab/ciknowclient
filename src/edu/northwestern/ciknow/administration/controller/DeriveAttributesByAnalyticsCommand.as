package edu.northwestern.ciknow.administration.controller
{
	...
	public class DeriveAttributesByAnalyticsCommand
	{
		private static var logger:ILogger = LogUtil.getLog(DeriveAttributesByAnalyticsCommand);
		...
		public function DeriveAttributesByAnalyticsCommand()
		{
		}
		
		public function execute(event:NodeEvent):AsyncToken{
			logger.info("Deriving attributes by analytics...");
			return service.deriveAttributeByAnalytics(event.data);
		}
		
		public function result(event:ResultEvent, trigger:NodeEvent):void{		
			logger.info("Derived attributes by analytics.");
