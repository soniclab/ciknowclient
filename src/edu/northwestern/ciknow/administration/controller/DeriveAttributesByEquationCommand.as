package edu.northwestern.ciknow.administration.controller
{
	...
	public class DeriveAttributesByEquationCommand
	{
		private static var logger:ILogger = LogUtil.getLog(DeriveAttributesByEquationCommand);
		...
		public function DeriveAttributesByEquationCommand()
		{
		}
		
		public function execute(event:NodeEvent):AsyncToken{
			logger.info("Deriving attributes by equation...");
			return service.deriveAttributeByEquation(event.data);
		}
		
		public function result(event:ResultEvent, trigger:NodeEvent):void{		
			logger.info("Derived attributes by equation.");
