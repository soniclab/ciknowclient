package edu.northwestern.ciknow.recommendation.controller
{
	...
	public class MahoutEvaluateRecommenderCommand
	{
		private static var logger:ILogger = LogUtil.getLog(MahoutEvaluateRecommenderCommand);
		...
		public function MahoutEvaluateRecommenderCommand()
		{
		...
		public function execute(event:RecommendationEvent):AsyncToken{
			...
			return service.evaluateMahoutRecommender(event.data);
		}
		
		public function result(event:ResultEvent, trigger:RecommendationEvent):void{
			logger.info("Evaluated Recommender.");			
			var e:RecommendationEvent = new RecommendationEvent(RecommendationEvent.EVALUATED_MAHOUT_RECOMMENDER);
			e.data = event.result;
			dispatcher(e);
