package edu.northwestern.ciknow.recommendation.controller
{
	...
	public class TeamAssembleCommand
	{
		private static var logger:ILogger = LogUtil.getLog(TeamAssembleCommand);
		...
		public function TeamAssembleCommand()
		{
			
		}
		
		public function execute(event:RecommendationEvent):AsyncToken{
			logger.info("Assembling Team ...");	
			return service.assembleTeam(event.data);
		}
		
		public function result(event:ResultEvent, trigger:RecommendationEvent):void{
			logger.info("Assembled Team.");			
			var e:RecommendationEvent = new RecommendationEvent(RecommendationEvent.ASSEMBLED_TEAM);
			e.data = event.result;
			dispatcher(e);
		}
